...
var _ = ginkgo.Describe("[Package Management]", ginkgo.Ordered, func() {
	...
	ginkgo.It("can deploy a subnet with subnet-evm version", func() {
		...
		gomega.Expect(utils.CheckOdysseyGoExists(binaryToVersion[utils.SoloOdygoKey])).Should(gomega.BeFalse())

		commands.CreateSubnetEvmConfigWithVersion(subnetName, utils.SubnetEvmGenesisPath, binaryToVersion[utils.SoloSubnetEVMKey1])
		deployOutput := commands.DeploySubnetLocallyWithVersion(subnetName, binaryToVersion[utils.SoloOdygoKey])
		...
		gomega.Expect(utils.CheckOdysseyGoExists(binaryToVersion[utils.SoloOdygoKey])).Should(gomega.BeTrue())
	...
	ginkgo.It("can deploy with multiple odysseygo versions", func() {
		// check odygo install precondition
		gomega.Expect(utils.CheckOdysseyGoExists(binaryToVersion[utils.MultiOdygo1Key])).Should(gomega.BeFalse())
		gomega.Expect(utils.CheckOdysseyGoExists(binaryToVersion[utils.MultiOdygo2Key])).Should(gomega.BeFalse())

		commands.CreateSubnetEvmConfigWithVersion(subnetName, utils.SubnetEvmGenesisPath, binaryToVersion[utils.MultiOdygoSubnetEVMKey])
		deployOutput := commands.DeploySubnetLocallyWithVersion(subnetName, binaryToVersion[utils.MultiOdygo1Key])
		rpcs, err := utils.ParseRPCsFromOutput(deployOutput)
		if err != nil {
			fmt.Println(deployOutput)
		}
		...
		// check odygo install
		gomega.Expect(utils.CheckOdysseyGoExists(binaryToVersion[utils.MultiOdygo1Key])).Should(gomega.BeTrue())
		gomega.Expect(utils.CheckOdysseyGoExists(binaryToVersion[utils.MultiOdygo2Key])).Should(gomega.BeFalse())

		commands.CleanNetwork()

		deployOutput = commands.DeploySubnetLocallyWithVersion(subnetName, binaryToVersion[utils.MultiOdygo2Key])
		rpcs, err = utils.ParseRPCsFromOutput(deployOutput)
		if err != nil {
			fmt.Println(deployOutput)
		}
		...
		// check odygo install
		gomega.Expect(utils.CheckOdysseyGoExists(binaryToVersion[utils.MultiOdygo1Key])).Should(gomega.BeTrue())
		gomega.Expect(utils.CheckOdysseyGoExists(binaryToVersion[utils.MultiOdygo2Key])).Should(gomega.BeTrue())
